        ...
        {res.items.map((product: products.Product) => (
          ...
          >
            ...
            <div className="flex justify-between">
              <span className="font-semibold text-bluey montserrat-font">{product.name}</span>
              <span className="poiret_one-font font-semibold">USD ${product.price?.price}</span>
            </div>
                ...
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    product.additionalInfoSections.find(
                      (section: any) => section.title === "Brand"
                    )?.description || ""
                  ),
                }}
              ></div>
            )}
            <button className="rounded-2xl ring-1 ring-bluey text-bluey w-max py-2 px-4 text-xs hover:bg-bluey hover:text-pinky transition duration-500">
              View Product
            </button>
          </Link>
        ))}
